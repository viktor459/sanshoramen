...
export async function POST(req: Request) {
  const { event_id, event_name, fname, lname, email, phone, guests } = await req.json();

  if (!event_id || !email || !fname) {
    return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
  }

  await supabase.from("waitlist").insert([{ event_id, event_name, fname, lname, email, phone, guests }]);
